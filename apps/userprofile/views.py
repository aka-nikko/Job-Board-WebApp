...
def list_jobs(request):
    ...
    return render(request, 'userprofile/list_jobs.html', {'userprofile': request.user.userprofile,'jobs': jobs})
...
def view_application(request, application_id):
    ...
    return render(request, 'userprofile/view_application.html', {'userprofile': request.user.userprofile,'application': application})

@login_required
def view_dashboard_job(request, job_id):
    job = get_object_or_404(Job, pk=job_id, created_by=request.user)

    return render(request, 'userprofile/view_dashboard_job.html', {'userprofile': request.user.userprofile,'job': job})

@login_required
def view_applied(request):
    jobs = Job.objects.all().order_by('-created_at')
    return render(request, 'userprofile/view_applied.html', {'userprofile': request.user.userprofile,'jobs': jobs})